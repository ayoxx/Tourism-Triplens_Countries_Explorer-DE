...
def transfer_minio_data_to_snowflake(bucket, file_key, target_table):
    ...
    try:
        snow_cursor.execute("USE SCHEMA TRIPLENS.RAW")
        
        snow_cursor.execute("CREATE OR REPLACE FILE FORMAT TRIPLENS_JSON_FMT TYPE = JSON")
        
        snow_cursor.execute("CREATE OR REPLACE STAGE TRIPLENS_STAGE FILE_FORMAT = TRIPLENS_JSON_FMT")
        ...
        snow_cursor.execute(f'PUT file://{local_temp_path} @TRIPLENS_STAGE AUTO_COMPRESS=TRUE OVERWRITE=TRUE')
        
        snow_cursor.execute(f'TRUNCATE TABLE {target_table};')
        snow_cursor.execute(f"""
            COPY INTO {target_table} (ingestion_time, src_file, payload)
            FROM (
